...
import {
    Search, RefreshCw, Clock, CheckCircle, FileText, FlaskConical, Clipboard, AlertCircle
} from 'lucide-react';
...
export const LabDashboard = () => {
    ...
    const openSubmitDialog = (order) => {
        ...
        setResultSummary('');
        setReportText('');
        // Generate a clean mock PDF filename based on the test name and patient
        ...
        setAttachmentUrl(`/reports/lab_${formattedTest}_${formattedPatient}.pdf`);
        setSubmitError(null);
    ...
    const handleSubmitResult = async (e) => {
        ...
        if (!resultSummary.trim() || !reportText.trim()) {
            setSubmitError('Please fill in both the summary and detailed report text.');
            return;
        }
        ...
        setSubmitError(null);
        try {
            await labApi.submitResult(selectedOrder.id, {
            ...
            console.error('Error submitting lab test result:', err);
            setSubmitError(err.response?.data?.detail || 'Failed to submit test results. Please check the inputs.');
        } finally {
            setSubmitting(false);
        }
    };
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                <Box>
                    ...
                    <Typography variant="body2" sx={{ color: 'text.secondary', fontWeight: 500 }}>
                        Manage diagnostic requests, analyze blood/pathology specimens, and upload clinical test results.
                    </Typography>
                </Box>
                <Button
                    variant="outlined"
            ...
            <Card sx={{ borderRadius: '24px', border: '1px solid', borderColor: 'divider', boxShadow: '0 8px 32px rgba(0,0,0,0.02)', overflow: 'hidden' }}>
                <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 3, pt: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                    <Tabs value={activeTab} onChange={handleTabChange} textColor="primary" indicatorColor="primary" sx={{
                        ...
                        <Tab label={`Pending Tests (${filteredPending.length})`} />
                        <Tab label={`Completed Reports (${filteredCompleted.length})`} />
                    </Tabs>
                    ...
                    {loading ? (
                        <Box sx={{ display: 'flex', justifyContent: 'center', alignItems: 'center', p: 8, gap: 2 }}>
                            ...
                            <Typography variant="body1" sx={{ color: 'text.secondary', fontWeight: 500 }}>
                                Fetching lab diagnostic records...
                            </Typography>
                        </Box>
                    ) : activeTab === 0 ? (
                        /* PENDING QUEUE TABLE */
                        filteredPending.length === 0 ? (
                            <Box sx={{ p: 8, textAlign: 'center' }}>
                                ...
                                <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
                                    Specimens or test requests ordered by doctors will appear here automatically.
                                </Typography>
                            </Box>
                        ) : (
                                ...
                                <Table>
                                    <TableHead sx={{ bgcolor: 'rgba(0, 106, 106, 0.02)' }}>
                                        <TableRow>
                                            ...
                                            <TableCell sx={{ fontWeight: 700 }}>Requested Test</TableCell>
                                            <TableCell sx={{ fontWeight: 700 }}>Ordering Doctor</TableCell>
                                            <TableCell sx={{ fontWeight: 700 }}>Order Date</TableCell>
                                            <TableCell sx={{ fontWeight: 700 }}>Clinical Notes</TableCell>
                                        ...
                                        {filteredPending.map((order) => (
                                            <TableRow key={order.id} hover sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                                                ...
                                                <TableCell sx={{ maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '0.85rem' }}>
                                                    {order.notes || <Typography variant="caption" sx={{ fontStyle: 'italic', color: 'text.secondary' }}>No details provided</Typography>}
                                                </TableCell>
                        ...
                        filteredCompleted.length === 0 ? (
                            <Box sx={{ p: 8, textAlign: 'center' }}>
                                ...
                                <Typography variant="body2" sx={{ color: 'text.secondary', mt: 0.5 }}>
                                    Completed tests with published results will show up in this history tab.
                                </Typography>
                            </Box>
                        ) : (
                            <TableContainer component={Paper} elevation={0} sx={{ border: 'none' }}>
                                <Table>
                                    <TableHead sx={{ bgcolor: 'rgba(0, 106, 106, 0.02)' }}>
                                        <TableRow>
                                            ...
                                            <TableCell sx={{ fontWeight: 700 }}>Results Summary</TableCell>
                                            <TableCell sx={{ fontWeight: 700 }}>Ordering Doctor</TableCell>
                ...
                <form onSubmit={handleSubmitResult}>
                    ...
                    <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                        ...
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12} sm={4}>
                                <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>ORDERED TEST</Typography>
                        ...
                        <TextField
                            ...
                            placeholder="e.g. HbA1c 6.8% - Elevated, Normal Hemoglobin levels"
                            value={resultSummary}
                        ...
                        <TextField
                            label="Detailed Lab Metrics / Findings"
                            placeholder="Enter the complete lab panel metrics, reference ranges, and analytical notes..."
                            value={reportText}
                        ...
                        <TextField
                            label="Mock Lab Report URL"
                            placeholder="e.g. /reports/cbc.pdf"
                            value={attachmentUrl}
                            onChange={(e) => setAttachmentUrl(e.target.value)}
                            fullWidth
                            variant="outlined"
                            InputProps={{ sx: { borderRadius: '12px' } }}
                            helperText="Simulated file system link to the PDF report file."
                        />
                    </DialogContent>
                    <DialogActions sx={{ p: 2.5, gap: 1 }}>
                        ...
                        <Button type="submit" variant="contained" disabled={submitting} sx={{ borderRadius: '100px', textTransform: 'none', fontWeight: 600, px: 3 }}>
                            {submitting ? 'Submitting...' : 'Publish Results'}
                        </Button>
                    </DialogActions>
                </form>
            </Dialog>

            {/* DIALOG 2: VIEW COMPLETED LAB REPORT */}
                ...
                <DialogContent dividers sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={4}>
                            ...
                            <Typography variant="body2" sx={{ fontWeight: 700, color: 'primary.main' }}>LABORATORY</Typography>
                        </Grid>
                        <Grid item xs={12} sm={4}>
                            <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>ORDERED TEST</Typography>
                    ...
                    <Box sx={{ bgcolor: 'action.hover', p: 2.5, borderRadius: '16px', border: '1px solid', borderColor: 'divider' }}>
                        <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', display: 'block', mb: 1 }}>DETAILED LABORATORY METRICS</Typography>
                    ...
                    <Grid container spacing={2}>
                        <Grid item xs={12} sm={6}>
                            <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary' }}>ANALYST SIGN-OFF</Typography>
                            <Typography variant="subtitle2" sx={{ fontWeight: 700 }}>{viewOrder?.result?.performed_by_name}</Typography>
                            <Typography variant="caption" sx={{ color: 'text.secondary' }}>Certified Lab Technician</Typography>
                        </Grid>
                        <Grid item xs={12} sm={6} align="right">
                            {viewOrder?.result?.attachment_url && (
                                <Box>
                                    <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', display: 'block', mb: 0.5 }}>ATTACHED LAB SHEET</Typography>
                                    <Button
                                        variant="text"
                                        color="primary"
